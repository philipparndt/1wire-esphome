...
const hexToBytes = (hex) => {
    ...
    for (let c = 0; c < hex.length; c += 2) {
        bytes.insert(0, parseInt(hex.slice(c, c + 2), 16))
    }
    return bytes;
}
...
const crc8 = (data) => {
    ...
    return (("0" + ((crc ^ xorOut) & 0xFFFF)
        .toString(16))
        .slice(-2))
        .toUpperCase()
}

export const toEspHomeId = (uid) => {
    const id = uid.replace(".", "")
    let split = id.match(/.{2}/g)
    split = split.reverse()

    const joined = split.join("")
    return `0x${crc8(joined)}${joined}`
}
